...
<script>
  ...
  export default {
    computed: mapGetters({
      ...
      error: 'income/show/error',
      loading: 'income/show/loading',
      item: 'income/show/item',
    }),
    methods: {
      deleteItem (item) {
        if (window.confirm('Are you sure you want to delete this item?'))
          this.$store.dispatch('income/del/delete', item).then(response => this.$router.push({ name: 'IncomeList' }));
      }
    },
    created () {
      this.$store.dispatch('income/show/retrieve', decodeURIComponent(this.$route.params.id));
    },
    beforeDestroy() {
      this.$store.dispatch('income/show/reset');
    }
  }
</script>
